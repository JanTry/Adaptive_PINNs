...
class AllenCahnProblem2D(ProblemInterface2D):
    def __init__(
        self,
        alpha: float = 1e-4,
        c: float = 5.0,
        ...
        self.c = torch.tensor(float(c), device=DEVICE)
        self.x_range = torch.tensor([-1.0, 1.0], device=DEVICE)
    ...
    def f_inner_loss(
        ...
        u = f(pinn, x, y)
        u_t = dfdy(pinn, x, y)
        u_xx = dfdx(pinn, x, y, order=2)
        return u_t - self.alpha * u_xx + self.c * (u**3 - u)

    def __initial_condition_loss(
        self, x: torch.Tensor, y: torch.Tensor, pinn: PINN_2D
    ) -> torch.Tensor:
        # enforce u(x,0) = x^2 cos(pi x)
        ...
        target = x0**2 * torch.cos(math.pi * x0)
        return (f(pinn, x0, y0) - target).pow(2).mean()

    def __periodic_x_loss(
        self, x: torch.Tensor, y: torch.Tensor, pinn: PINN_2D
    ) -> torch.Tensor:
        # Pair left (x=-1) and right (x=1) boundary points by matching/sorting in t.
        tol = 1e-8
        ...
        # value periodicity: u(-1,t) == u(1,t)
        u_left = f(pinn, xl, tl)
        u_right = f(pinn, xr, tr)
        loss_u = (u_left - u_right).pow(2).mean()

        # derivative periodicity u_t(-1,t) == u_t(1,t)
        ut_left = dfdy(pinn, xl, tl)
        ut_right = dfdy(pinn, xr, tr)
        loss_ut = (ut_left - ut_right).pow(2).mean()
